...
const PAGE_META: Record<string, { pt: { title: string; description: string }; ogPrefix: string }> =
  {
    "index.html": {
      pt: {
...
async function processStaticPages() {
  for (const [filename, meta] of Object.entries(PAGE_META)) {
    ...
    // Generate PT variant
    let ptHtml = html;
    ptHtml = replaceOgImage(ptHtml, `/og/${meta.ogPrefix}-en.png`, `/og/${meta.ogPrefix}-pt.png`);
    ...
    ptHtml = replaceMetaContent(ptHtml, "twitter:description", meta.pt.description);
...
async function processBlogPosts() {
  ...
  for (const dir of slugDirs) {
    ...
    // Generate PT variant
    let ptHtml = html;
    ptHtml = replaceOgImage(ptHtml, `/og/${slug}-en.png`, `/og/${slug}-pt.png`);
