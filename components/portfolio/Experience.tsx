...
import { DATA, STATIC_EXPERIENCE } from "@/lib/data/content";
import type { Language } from "@/lib/types";
import { formatDate, getDuration } from "@/lib/helpers";
	...
	return (
		...
		>
			<div className="max-w-4xl mx-auto">
				<h3 className="text-xs font-mono text-zinc-500 dark:text-zinc-500 mb-16 uppercase tracking-widest">
					{t.experience.title}
				</h3>
						...
						<div key={companyData.id} className="relative pb-24 last:pb-0">
							...
							<div className="pl-6 border-l-2 border-zinc-100 dark:border-zinc-900 ml-6 space-y-16">
								{companyData.positions.map((position) => {
									const localizedPos =
										localizedCompany.positions[
											companyData.positions.indexOf(position)
										];
									...
									return (
										<div
											key={`${companyData.id}-${position.startDate}`}
