...
const Navbar: React.FC<NavbarProps> = ({
  ...
  const navbarClass = useMemo(() => {
    const isScrolled = scrollY > 20 || pathname !== "/";
  ...
  // Memoize mobile nav click handler
  ...
  const mobileOverlay = (
    <nav
      className={`fixed inset-0 bg-white dark:bg-black z-[60] flex flex-col items-center justify-center transition-all duration-300 ease-in-out lg:hidden ${
...
export default React.memo(Navbar, (prev, next) => {
  ...
  return (
    ...
    prev.lang === next.lang &&
    prev.isSubdomain === next.isSubdomain
  );
});
